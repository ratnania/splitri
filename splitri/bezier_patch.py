...
class bezier_patch(object):
    def __init__(self, degree=None, vertices=None, control=None, method="uniform"):
        # creates a B-net given the 3 summets of a triangle and a total degree
        # A = vertices[0,:]
        # B = vertices[1,:]
        # C = vertices[2,:]
        self._array     = None
        ...
        self._init_global_id(self.degree)
    ...
    def global_id(self, i, j, k):
        return self._global_id[i,j,k]

    def _init_global_id(self, degree):
        self._global_id = np.zeros((degree+1, degree+1, degree+1) \
                                   , dtype=np.int32)
        ...
        for j in range(0, degree+1):
            k = degree - j
            ids_on_edge.append(self._global_id[i,j,k])
    ...
    def _create_triangulation(self):
        ...
        for i in range(0, self.degree+1-1):
            for j in range(0, self.degree+1-i-1):
                k = self.degree - i - j
                I1 = self.global_id(i,j,k)
                I2 = self.global_id(i,j+1,k-1)
                I3 = self.global_id(i+1,j,k-1)
#                print "UP   ", [I1, I2, I3]
...
#                print "DOWN ", [I1, I2, I3]
    ...
    def extract_edge(self, edge_id):
        ids = self._ids_on_edges[edge_id]
        points = self.array[ids,...]
